...
$targetDomain = Read-Host "Select the Exchange Online domain to migrate ? [Default : reliancecloud.mail.onmicrosoft.com] "
if ($targetDomain -like ""){$targetDomain = "reliancecloud.mail.onmicrosoft.com"}
...
$csv | ForEach-Object {
    ...
    New-MoveRequest -Identity $_.SamAccountName -Remote -RemoteHostName $RemoteHost -TargetDeliveryDomain $targetDomain -RemoteCredential $Localcred -BadItemLimit 200 -AcceptLargeDataLoss -SuspendWhenReadyToComplete | Out-Null
